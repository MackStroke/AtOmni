<?php
$file = 'resources/views/admin/homepage-sections/edit.blade.php';
...
$tagHtmlNew = <<<EOD
                      @php
                          \$selectedTags = old('filters.tag_ids', \$homepageSection->filters['tag_ids'] ?? []);
                          // Fallback for older entries using tag_id
                          if (empty(\$selectedTags) && \$homepageSection->tag_id) {
                              \$selectedTags = [\$homepageSection->tag_id];
                          }
                      @endphp
