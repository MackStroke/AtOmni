...
<form method="POST" action="{{ route('admin.posts.store') }}" id="post-form" autocomplete="off">
    @csrf
        ...
        <div class="editor-main space-y-5">
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    ...
                    <span id="tldr-counter" class="text-xs text-text-muted">0 / 300</span>
                </div>
                <textarea id="tldr" name="tldr" rows="2" maxlength="300"
                          ...
                          oninput="document.getElementById('tldr-counter').innerText = this.value.length + ' / 300'">{{ old('tldr') }}</textarea>
            ...
            <div>
                <textarea id="excerpt" name="excerpt" rows="2"
                          class="w-full px-4 py-2.5 rounded-lg bg-navy-800/50 border border-navy-700/50 text-text-primary text-sm placeholder-text-muted focus:border-electric focus:outline-none focus:ring-1 focus:ring-electric/30 transition-colors resize-none"
                          placeholder="Short summary (excerpt)…">{{ old('excerpt') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="20"
                          ...
                          placeholder="Write your article content…">{{ old('content') }}</textarea>
        ...
        <div class="editor-sidebar space-y-4" id="editor-sidebar">
            ...
            <div class="glass-card rounded-xl p-4 space-y-3">
                ...
                <div id="featured-image-preview" class="w-full h-32 rounded-lg bg-navy-900 border-2 border-dashed border-navy-700/50 flex flex-col items-center justify-center relative overflow-hidden group">
                    <div id="featured-image-placeholder" class="text-center {{ old('featured_image') ? 'hidden' : '' }}">
                    ...
                    <img loading="lazy" id="featured-image-img" alt="Featured image preview" src="{{ old('featured_image') ? (\Illuminate\Support\Str::startsWith(old('featured_image'), 'http') ? old('featured_image') : asset('storage/' . old('featured_image'))) : '' }}" 
                         class="absolute inset-0 w-full h-full object-cover {{ old('featured_image') ? '' : 'hidden' }}">
                    <button type="button" aria-label="Remove featured image" onclick="removeFeaturedImage()" id="featured-image-remove" class="absolute top-1.5 right-1.5 p-1 bg-navy-950/80 text-rose-400 hover:text-rose-300 rounded-md backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity focus:outline-none {{ old('featured_image') ? '' : 'hidden' }}">
            ...
            <div class="glass-card rounded-xl p-4 space-y-3">
                ...
                <div id="publish-date-picker" class="hidden">
                ...
                <div id="featured_until_container" class="{{ old('is_featured') ? '' : 'hidden' }}">
                    ...
                    <input type="datetime-local" id="featured_until" name="featured_until" value="{{ old('featured_until') }}"
                ...
                <div class="pt-2 border-t border-navy-700/50 space-y-2">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" id="kill_switch" name="kill_switch" value="1" {{ old('kill_switch') ? 'checked' : '' }}
                    ...
                    <div id="redirect_url_container" class="{{ old('kill_switch') ? '' : 'hidden' }}">
                        <label for="redirect_url" class="block text-[10px] font-medium text-text-muted mb-0.5">Redirect URL (Optional)</label>
                        <input type="url" id="redirect_url" name="redirect_url" value="{{ old('redirect_url') }}" placeholder="https://..."
                    ...
                    <select name="locations[]" id="locations" multiple class="w-full px-2 py-1.5 rounded-lg bg-navy-800/50 border border-navy-700/50 text-xs text-text-primary focus:border-electric focus:outline-none transition-colors">
                        ...
                        @foreach($locations as $country)
                            <optgroup label="{{ $country->name }}">
                                <option value="{{ $country->id }}" {{ in_array($country->id, old('locations', [])) ? 'selected' : '' }}>{{ $country->name }} (Country)</option>
                                @foreach($country->children as $state)
                                    <option value="{{ $state->id }}" {{ in_array($state->id, old('locations', [])) ? 'selected' : '' }}>{{ $state->name }}</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                    </select>
            ...
            {{-- Content Analysis (Sticky in sidebar) --}}
