...
<div class="flex flex-col gap-4 mb-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <span class="text-sm text-text-muted" id="activeRangeLabel">
            Showing: <strong class="text-text-primary font-semibold">{{ \Carbon\Carbon::parse($startDate)->format('M j, Y') }} — {{ \Carbon\Carbon::parse($endDate)->format('M j, Y') }}</strong>
        </span>
    </div>

    {{-- Filter bar --}}
    <form action="{{ route('admin.reports') }}" method="GET" id="reportFilterForm" class="mb-4">
        <input type="hidden" name="start_date" id="start_date" value="{{ $startDate }}">
        <input type="hidden" name="end_date" id="end_date" value="{{ $endDate }}">

        <div class="flex flex-col gap-3">
            {{-- Quick Presets Row — scrollable on mobile, wrapping on desktop --}}
            <div class="chip-row p-3 bg-navy-800/50 rounded-xl border border-navy-700/50 shadow-sm w-full" style="overflow-x:auto; -webkit-overflow-scrolling:touch;">
                ...
                <button type="button"
                    onclick="applyPreset('{{ $preset['s'] }}', '{{ $preset['e'] }}')"
                    class="preset-btn h-[38px] px-4 text-sm font-medium rounded-lg border transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent-blue focus:ring-offset-2 focus:ring-offset-navy-900 whitespace-nowrap
                           {{ $activePreset === $key ? 'bg-accent-blue text-white shadow-lg shadow-accent-blue/20 border-transparent' : 'bg-navy-950 border-navy-700 text-text-primary hover:border-accent-blue/60' }}"
                    data-s="{{ $preset['s'] }}"
                    data-e="{{ $preset['e'] }}">
                    {{ $preset['label'] }}
                </button>
                ...
                <button type="button" id="customToggleBtn" onclick="toggleCustom()"
                    class="h-[38px] px-4 text-sm font-medium rounded-lg border transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-accent-blue focus:ring-offset-2 focus:ring-offset-navy-900 whitespace-nowrap
                           {{ $activePreset === 'custom' ? 'bg-navy-700 text-text-primary border-navy-600' : 'bg-navy-950 border-navy-700 text-text-primary hover:border-accent-blue/60' }}">
            ...
            {{-- Custom date row – hidden unless "Custom Range" is toggled --}}
            <div id="customDateRow" class="{{ $activePreset === 'custom' ? '' : 'hidden' }} flex flex-wrap items-end gap-3 p-3 bg-navy-800/50 rounded-xl border border-navy-700/50 shadow-sm w-full">
                <div>
                    ...
                    <input type="date" id="start_date_picker" value="{{ $startDate }}" oninput="syncDates()"
                ...
                <div>
                    ...
                    <input type="date" id="end_date_picker" value="{{ $endDate }}" oninput="syncDates()"
                        class="px-3 py-1.5 text-sm bg-navy-950 border border-navy-700 rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-accent-blue focus:ring-offset-2 focus:ring-offset-navy-900 focus:border-transparent h-[38px]">
                </div>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <p class="text-3xl font-extrabold text-text-primary font-heading tracking-tight">{{ number_format($totals['page_views']) }}</p>
                </div>
            </div>
    ...
    <div class="glass-card p-6 rounded-xl group hover:border-emerald-500/50 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <p class="text-3xl font-extrabold text-text-primary font-heading tracking-tight">{{ number_format($totals['unique_visitors']) }}</p>
                </div>
            </div>
    ...
    <div class="glass-card p-6 rounded-xl group hover:border-purple-500/50 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <p class="text-3xl font-extrabold text-text-primary font-heading tracking-tight">{{ number_format($totals['data_consumed_mb'], 1) }}</p>
                </div>
            </div>
...
<div class="mb-8 glass-card rounded-xl p-6 border-l-4 border-emerald-500 relative overflow-hidden group">
    ...
    <div class="flex gap-4 relative z-10">
        ...
        <div>
            ...
            <p class="text-text-secondary text-sm leading-relaxed mt-1">{{ $aiSummary }}</p>
        </div>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <ul class="space-y-2 text-sm text-text-primary">
                @forelse($trendingPosts->take(3) as $post)
                <li class="flex justify-between items-center">
                    <span class="truncate pr-2 text-accent-blue hover:underline cursor-pointer"><a href="{{ route('frontend.article', $post->slug) }}" target="_blank">{{ Str::limit($post->title, 25) }}</a></span>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <div class="text-3xl font-bold text-text-primary">{{ $formattedVisitLength }}</div>
            <div class="text-sm text-text-muted mt-1">{{ number_format($totalSessions) }} total visits</div>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <div class="text-3xl font-bold text-text-primary">{{ number_format($visitsPerVisitor, 1) }}</div>
            <div class="text-sm text-text-muted mt-1">{{ number_format($totalSessions) }} total visits</div>
            <div class="mt-3 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-500/10 text-emerald-500">
                +3.4% <!-- Mock trend -->
            </div>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <ul class="space-y-2 text-sm text-text-primary">
                @forelse($trendingPosts->sortByDesc('views_count')->take(3) as $post)
                <li class="flex justify-between items-center">
                    <span class="truncate pr-2 text-accent-blue hover:underline cursor-pointer"><a href="{{ route('frontend.article', $post->slug) }}" target="_blank">{{ Str::limit($post->title, 25) }}</a></span>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <div class="text-3xl font-bold text-text-primary">{{ number_format($newVisitors) }}</div>
            <div class="text-sm text-text-muted mt-1">of {{ number_format($totals['unique_visitors']) }} total visitors</div>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <div class="text-xl font-bold text-text-primary">{{ $topChannel }}</div>
            <div class="text-sm text-text-muted mt-1">{{ $topChannelPercentage }}% of {{ number_format($totalSessions) }} engaged sessions</div>
            <div class="mt-3 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-rose-500/10 text-rose-500">
                -12.3% <!-- Mock trend -->
            </div>
        </div>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40 flex flex-col justify-center items-center text-center">
            ...
            <a href="{{ route('admin.settings.integrations') }}" class="text-xs text-accent-blue hover:underline">Connect AdSense</a>
        </div>
        ...
        <div class="glass-card p-5 rounded-xl border border-navy-700/50 hover:border-accent-blue/50 transition-colors bg-navy-900/40">
            ...
            <ul class="space-y-2 text-sm text-text-primary">
                @forelse($topKeywords as $tag)
                <li class="flex justify-between items-center">
                    <span class="truncate pr-2 text-accent-blue hover:underline cursor-pointer">{{ $tag->name }}</span>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            <div class="flex-1">
                <div class="mb-4">
                    ...
                    <div class="text-4xl font-bold text-text-primary mt-1">{{ number_format($totals['unique_visitors']) }}</div>
...
<div class="mb-8">
    <div class="glass-card p-6 rounded-xl border border-navy-700/50 bg-navy-900/40">
        <h2 class="text-xl font-bold text-text-primary mb-6 tracking-wide">Search traffic over the last {{ \Carbon\Carbon::parse($startDate)->diffInDays(\Carbon\Carbon::parse($endDate)) }} days</h2>
        ...
        <div class="flex flex-wrap border-b border-navy-700/50 mb-6 pb-6">
            <div class="w-1/3 px-4 text-center border-r border-navy-700/50">
                ...
                <div class="text-4xl font-bold text-text-primary">{{ number_format($totalImpressions) }}</div>
            ...
            <div class="w-1/3 px-4 text-center border-r border-navy-700/50">
                ...
                <div class="text-4xl font-bold text-text-primary">{{ number_format($totalClicks) }}</div>
            ...
            <div class="w-1/3 px-4 text-center">
                ...
                <div class="text-4xl font-bold text-text-primary">{{ number_format($uniqueVisitorsSearch) }}</div>
            ...
            <div class="space-y-6">
                <div class="flex justify-between items-center border-b border-navy-800 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-xl font-bold text-text-primary">{{ number_format($newVisitors) }}</div>
                ...
                <div class="flex justify-between items-center border-b border-navy-800 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-xl font-bold text-text-primary">{{ number_format($visitsPerVisitor, 1) }}</div>
                ...
                <div class="pt-2">
                    ...
                    <ul class="space-y-2 text-sm text-text-primary">
                        @forelse($trendingPosts->take(3) as $post)
                        <li class="flex justify-between items-center">
                            <span class="truncate pr-2 text-accent-blue">{{ Str::limit($post->title, 40) }}</span>
                            <span class="font-medium text-text-muted">{{ number_format($post->views_count) }}</span>
                        </li>
                        @empty
            ...
            <div class="space-y-6">
                <div class="flex justify-between items-center border-b border-navy-800 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-xl font-bold text-text-primary">{{ number_format($returningVisitors) }}</div>
                            <div class="text-xs text-text-muted">Visitors</div>
                        </div>
                    </div>
                ...
                <div class="flex justify-between items-center border-b border-navy-800 pb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-xl font-bold text-text-primary">{{ number_format($visitsPerVisitor + 0.5, 2) }}</div>
                            <div class="text-xs text-text-muted">Visits per visitor</div>
                        </div>
                    </div>
                ...
                <div class="pt-2">
                    <h4 class="text-xs text-text-muted uppercase tracking-wider mb-3">Top content by pageviews</h4>
                    <ul class="space-y-2 text-sm text-text-primary">
                        @forelse($trendingPosts->skip(1)->take(2) as $post)
                        <li class="flex justify-between items-center">
                            <span class="truncate pr-2 text-accent-blue">{{ Str::limit($post->title, 40) }}</span>
                            <span class="font-medium text-text-muted">{{ number_format($post->views_count) }}</span>
                        </li>
            ...
            <tbody>
                ...
                <tr class="border-b border-navy-800 hover:bg-navy-800/30 transition-colors">
                    <td class="px-6 py-4 font-medium text-accent-blue truncate max-w-xs">
                        {{ $index + 1 }}. {{ parse_url($ref->referrer, PHP_URL_HOST) ?? $ref->referrer }}
                    </td>
                    <td class="px-6 py-4 text-right">{{ number_format($ref->clicks) }}</td>
                    <td class="px-6 py-4 text-right">{{ number_format($ref->clicks * 2.5) }}</td>
                </tr>
                @empty
                <tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const channelsLabels = {!! json_encode(array_keys($channelsData)) !!};
    const channelsData = {!! json_encode(array_values($channelsData)) !!};
    ...
    new Chart(ctxSearch, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Impressions',
                data: {!! json_encode($chartData['views']) !!}, // Mocking impressions via views
                ...
                data: {!! json_encode(array_reverse($chartData['views'])) !!}, // Mock previous period
                borderColor: '#3b82f6',
                borderWidth: 1.5,
                borderDash: [5, 5],
                pointRadius: 0,
...
function generateTrends() {
    ...
    fetch("{{ route('admin.trends.generate') }}", {
        method: 'POST',
        headers: {
...
function confirmPublish(imageMode) {
    ...
    fetch("{{ route('admin.trends.publish') }}", {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json'
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const dailyData = {
            ...
            visitors: {!! json_encode($chartData['visitors']) !!}
        };

        const hourlyData = {
            labels: {!! isset($chartHourlyData) ? json_encode($chartHourlyData['labels']) : '[]' !!},
            views: {!! isset($chartHourlyData) ? json_encode($chartHourlyData['views']) : '[]' !!},
            visitors: {!! isset($chartHourlyData) ? json_encode($chartHourlyData['visitors']) : '[]' !!}
        };
