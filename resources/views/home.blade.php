...
<script type="application/ld+json">
{
    ...
    "itemListElement": [
        @foreach($featuredPosts as $i => $fp)
        {
            "@@type": "ListItem",
            "position": {{ $i + 1 }},
            "url": "{{ route('frontend.article', $fp->slug) }}",
            "name": "{{ e($fp->title) }}"
        }{{ !$loop->last ? ',' : '' }}
        @endforeach
    ]
...
@section('content')
<h1 class="sr-only">{{ $siteName }} - {{ $tagline }}</h1>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        {{-- ── Featured Article Carousel (3/5 width) ── --}}
        <div class="lg:col-span-3">
            @if(collect($featuredPosts)->isNotEmpty())
            <div class="relative w-full h-full min-h-[320px] lg:min-h-[420px] rounded-2xl overflow-hidden group/carousel" id="featured-carousel">
                ...
                <a href="{{ route('frontend.article', $featuredPost->slug) }}" 
                   class="carousel-slide absolute inset-0 transition-opacity duration-1000 block focus:outline-none {{ $index === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0' }}">
                    <article class="relative group rounded-2xl overflow-hidden cursor-pointer h-full w-full">
                        <div class="absolute inset-0 bg-navy-800">
                            {{-- First slide: eager + high priority = LCP candidate --}}
                            {{-- Other slides: lazy = don't block initial load --}}
                            <img
                        ...
                        <div class="relative flex flex-col justify-end h-full p-6 lg:p-8" style="z-index:10">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <span class="px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider bg-electric text-white">{{ $featuredPost->category->name }}</span>
                                @endif
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-navy-700 overflow-hidden">
                                    @if($featuredPost->author && $featuredPost->author->avatar)
                                        <img loading="lazy" src="{{ \Illuminate\Support\Facades\Storage::url($featuredPost->author->avatar) }}" alt="{{ $featuredPost->author->name }}" class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center text-xs font-bold text-white bg-electric">{{ substr($featuredPost->author?->name ?? 'A', 0, 1) }}</div>
                                    @endif
                                </div>
                                <div class="text-sm">
                                    <span class="text-text-primary font-medium">{{ $featuredPost->author?->name ?? 'Staff Writer' }}</span>
                                    <span class="text-text-muted mx-1.5">·</span>
                                    <span class="text-text-muted">{{ $featuredPost?->reading_time ?? 5 }} min read</span>
                                    <span class="text-text-muted mx-1.5">·</span>
                                    <span class="text-text-muted">{{ $featuredPost->published_at?->diffForHumans() ?? 'Recently' }}</span>
                                </div>
                            </div>
                        </div>
                    </article>
                ...
                {{-- Carousel Indicators — meets 44px touch target via padding --}}
                @if(collect($featuredPosts)->count() > 1)
                <div class="absolute bottom-4 right-6 flex items-center gap-1 z-20">
                    @foreach($featuredPosts as $index => $post)
                    <button class="carousel-indicator group relative flex items-center justify-center w-6 h-6 rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-white" aria-label="Slide {{ $index + 1 }}">
                        <span class="block rounded-full transition-all duration-300 {{ $index === 0 ? 'bg-electric w-5 h-2' : 'bg-white/40 group-hover:bg-white/70 w-2 h-2' }}"></span>
                    </button>
                    @endforeach
                </div>
        ...
        {{-- ── 4 Secondary Articles (2/5 width, stacked) ── --}}
        <div class="lg:col-span-2 flex flex-col gap-4">
            {{-- Visually-hidden heading to maintain h2→h3 hierarchy --}}
            <h2 class="sr-only">Top Stories</h2>
            @forelse($secondaryPosts ?? [] as $post)
                <?php /** @var \App\Models\Post $post */ ?>
                <a href="{{ route('frontend.article', $post->slug) }}" class="flex gap-3 glass-card rounded-xl p-3 group cursor-pointer hover:bg-navy-800/60 transition-colors">
                    ...
                    <div class="flex flex-col justify-center min-w-0">
                        ...
                        @endif
                        <h3 class="text-sm font-semibold text-text-primary leading-snug line-clamp-2 group-hover:text-electric-light transition-colors">{{ $post->title }}</h3>
                        <span class="text-xs text-text-muted mt-1">{{ $post?->reading_time ?? 5 }} min read</span>
                    </div>
                </a>
            @empty
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
    <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div class="flex items-center gap-2 overflow-x-auto scroll-hide pb-3 pt-1 px-1 -mx-1 flex-1 w-full">
            <a href="{{ route('home') }}" class="shrink-0 px-4 py-2 rounded-full text-sm font-semibold bg-electric text-white shadow-lg shadow-electric/20">All</a>
            @foreach($categories as $cat)
                <a href="{{ route('category', $cat->slug) }}" class="shrink-0 px-4 py-2 rounded-full text-sm font-medium text-text-secondary bg-navy-800 hover:bg-navy-700 hover:text-text-primary transition-colors">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
            ...
            <select id="location-filter" onchange="if(this.value) window.location.href='/location/'+this.value;" class="w-full appearance-none px-4 py-2 rounded-full text-sm font-medium text-text-primary bg-navy-800 hover:bg-navy-700 transition-colors border border-navy-700 focus:outline-none focus:border-electric focus:ring-1 focus:ring-electric cursor-pointer">
                ...
                @foreach($locations as $country)
                    <optgroup label="{{ $country->name }}">
                        <option value="{{ $country->slug }}">{{ $country->name }} (Country)</option>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($latestPosts ?? [] as $post)
            <?php /** @var \App\Models\Post $post */ ?>
            <a href="{{ route('frontend.article', $post->slug) }}" class="block">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-6 h-6 rounded-full bg-navy-700 overflow-hidden">
                                @if($post->author && $post->author->avatar)
                                    <img loading="lazy" src="{{ \Illuminate\Support\Facades\Storage::url($post->author->avatar) }}" alt="{{ $post->author?->name ?? 'Author' }}" class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center text-xs font-bold text-white bg-electric">{{ substr($post->author?->name ?? 'A', 0, 1) }}</div>
                                @endif
                            </div>
    ...
    {{-- Real Pagination --}}
    @if($latestPosts instanceof \Illuminate\Pagination\AbstractPaginator && $latestPosts->hasPages())
    <div class="mt-10">
        {{ $latestPosts->links('partials.pagination') }}
    </div>
    @endif
</section>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    <div class="flex items-center gap-3 mb-6">
        <div class="w-1 h-7 bg-electric rounded-full"></div>
        <h2 class="font-heading font-bold text-2xl text-text-primary">{{ $authorSectionTitle ?? 'Selected Author' }}</h2>
    </div>
    
    <div class="bg-navy-900/50 rounded-3xl p-6 border border-navy-800">
        {{-- Top 3 large cards --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            @foreach($authorSectionPosts->take(3) as $post)
            <div class="glass-card rounded-2xl overflow-hidden group flex flex-col">
                <a href="{{ route('frontend.article', $post->slug) }}" class="block relative aspect-video overflow-hidden">
                    @if($post->featured_image)
                        <img src="{{ $post->featuredImageUrl() }}" alt="{{ $post->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    @else
                        <img src="{{ asset('images/atomni-placeholder.svg') }}" alt="Placeholder" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-80">
                ...
                <div class="p-5 flex flex-col flex-1">
                    <h3 class="font-heading font-bold text-lg text-text-primary line-clamp-2 group-hover:text-electric transition-colors">
                        <a href="{{ route('frontend.article', $post->slug) }}">{{ $post->title }}</a>
                    </h3>
                </div>
            </div>
            @endforeach
        ...
        <div class="bg-navy-800 rounded-2xl p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-navy-700">
                @foreach($authorSectionPosts->skip(3)->take(3) as $post)
                <a href="{{ route('frontend.article', $post->slug) }}" class="px-4 py-2 hover:text-electric transition-colors group flex items-start gap-3">
                    <div class="mt-1 w-2 h-2 rounded-full bg-electric/50 group-hover:bg-electric transition-colors shrink-0"></div>
                    <span class="font-heading font-medium text-sm text-text-primary group-hover:text-electric leading-snug line-clamp-3">{{ $post->title }}</span>
                </a>
                @endforeach
            </div>
        </div>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2">
            ...
            <div class="space-y-5">
                @forelse($editorPicks ?? [] as $post)
                    <?php /** @var \App\Models\Post $post */ ?>
                    <a href="{{ route('frontend.article', $post->slug) }}" class="flex gap-4 glass-card rounded-xl p-4 group cursor-pointer hover:bg-navy-800/60 transition-colors">
        ...
        <aside class="space-y-6">
            ...
            <div class="glass-card rounded-xl p-6">
                ...
                <form action="{{ route('subscribe') }}" method="POST" class="space-y-3" id="sidebar-subscribe-form">
                    @csrf
            ...
            <div class="glass-card rounded-xl p-6 text-center">
                ...
                <a href="{{ route('explore') }}" class="w-full inline-block px-4 py-2.5 rounded-lg bg-navy-800 border border-navy-700 text-text-primary text-sm font-semibold hover:bg-electric hover:text-white hover:border-electric transition-colors shadow-sm">
                    Browse All Topics
                </a>
            </div>
        </aside>
    ...
    <div class="border-b border-navy-700/50 pb-12 last:border-0 last:pb-0">
        <div class="flex items-center gap-3 mb-6">
            <h2 class="font-heading font-bold text-xl text-text-primary uppercase tracking-wide flex items-center gap-2">
                <a href="{{ route('category', $cat->slug) }}" class="hover:text-electric transition-colors">{{ $cat->name }}</a>
                <svg class="w-5 h-5 text-electric" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </h2>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            ...
            <a href="{{ route('frontend.article', $post->slug) }}" class="block group flex flex-col">
                <div class="relative aspect-[4/3] rounded-xl overflow-hidden mb-3">
                    @if($post->featured_image)
                        <img src="{{ $post->featuredImageUrl() }}" alt="{{ $post->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    @else
...
{{-- ─────────────────────────────────────────────────────────────────
     │  SHORT VIDEOS                                               │
     ───────────────────────────────────────────────────────────────── --}}
@if(isset($shortVideos) && $shortVideos->count() > 0)
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    <div class="flex items-center gap-3 mb-6">
    ...
    <div class="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 scroll-hide -mx-4 px-4 sm:mx-0 sm:px-0">
        @foreach($shortVideos as $post)
        <a href="{{ route('frontend.article', $post->slug) }}" class="snap-start shrink-0 w-[160px] block group">
...
{{-- ─────────────────────────────────────────────────────────────────
     │  TOP VIRAL VIDEOS (Long Form)                               │
     ───────────────────────────────────────────────────────────────── --}}
@if(isset($viralVideos) && $viralVideos->count() > 0)
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    <div class="flex items-center gap-3 mb-6">
    ...
    <div class="bg-navy-900/30 rounded-3xl p-6 border border-navy-800/50">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            @php $firstVideo = $viralVideos->first(); @endphp
            {{-- Big Video (Left) --}}
            <button type="button" @click="$dispatch('open-video-modal', { url: '{{ $firstVideo->redirect_url ?? '' }}', content: `{{ addslashes($firstVideo->content) }}` })" class="block group text-left w-full">
                <div class="relative aspect-video rounded-2xl overflow-hidden mb-4 border border-navy-700/50">
                    @if($firstVideo->featured_image)
                        <img src="{{ Storage::url($firstVideo->featured_image) }}" alt="{{ $firstVideo->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                    @else
                        <img src="{{ asset('images/atomni-placeholder.svg') }}" alt="Placeholder" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-80">
                    @endif
            ...
            @if($viralVideos->count() > 1)
            <div class="flex flex-col gap-4">
                @foreach($viralVideos->skip(1)->take(4) as $video)
                <button type="button" @click="$dispatch('open-video-modal', { url: '{{ $video->redirect_url ?? '' }}', content: `{{ addslashes($video->content) }}` })" class="group flex gap-4 items-center text-left w-full hover:bg-navy-800/40 p-2 rounded-xl transition-colors">
                    <div class="w-32 sm:w-40 aspect-video shrink-0 rounded-lg overflow-hidden relative border border-navy-700/50">
                        @if($video->featured_image)
                            <img src="{{ Storage::url($video->featured_image) }}" alt="{{ $video->title }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                        @else
                            <img src="{{ asset('images/atomni-placeholder.svg') }}" alt="Placeholder" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-80">
                        @endif
...
<section id="newsletter" class="bg-navy-900/80 border-y border-navy-700/30 py-16">
    <div class="max-w-2xl mx-auto px-4 text-center">
        ...
        <form action="{{ route('subscribe') }}" method="POST" class="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" id="footer-subscribe-form">
            @csrf
