...
<footer class="border-t border-navy-700/30 bg-navy-900/50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {{-- TOP ROW: Logo, Newsletter, Socials --}}
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pb-8 border-b border-navy-700/30 light:border-slate-200">
            ...
            <div class="flex flex-col md:flex-row md:items-center gap-6 lg:gap-12 w-full lg:w-auto">
                {{-- Logo --}}
                <a href="{{ route('home') }}" class="shrink-0 focus:outline-none focus:ring-2 focus:ring-electric rounded block">
                    @if($logoLight && $logoDark)
                        <img loading="lazy" src="{{ \Illuminate\Support\Facades\Storage::url($logoLight) }}" alt="{{ $siteName }}" class="h-9 w-auto object-contain logo-light">
                        <img loading="lazy" src="{{ \Illuminate\Support\Facades\Storage::url($logoDark) }}" alt="{{ $siteName }}" class="h-9 w-auto object-contain logo-dark">
                    @elseif($logoLight || $logoDark)
                        <img loading="lazy" src="{{ \Illuminate\Support\Facades\Storage::url($logoLight ?: $logoDark) }}" alt="{{ $siteName }}" class="h-9 w-auto object-contain">
                    @else
                        <img loading="lazy" src="{{ asset('images/atomni-logo-light.svg') }}" alt="{{ $siteName }}" class="h-9 w-auto object-contain logo-light">
                        <img loading="lazy" src="{{ asset('images/atomni-logo-dark.svg') }}" alt="{{ $siteName }}" class="h-9 w-auto object-contain logo-dark">
                    @endif
                </a>
                ...
                <div class="flex-1 w-full max-w-md">
                    ...
                    <form action="{{ route('subscribe') }}" method="POST" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-0">
                        @csrf
        ...
        {{-- MIDDLE GRID: DB-driven Footer Columns --}}
        <div class="py-10 border-b border-navy-700/30 light:border-slate-200">
            ...
            @if($hasDynamicMenus)
                @php $catCount = $footerCategories ? optional($footerCategories->rootItems)->count() : 0; @endphp
                <div class="grid grid-cols-1 md:grid-cols-{{ min($footerColumns->count() + ($catCount > 6 ? 1 : 0), 4) }} gap-8">
                    @foreach($footerColumns as $footerMenu)
                        @if($footerMenu)
                            <div class="{{ $footerMenu->location === 'footer_categories' && optional($footerMenu->rootItems)->count() > 6 ? 'md:col-span-2' : '' }}">
                                <h3 class="text-[13px] font-bold text-text-primary light:text-slate-900 mb-4 uppercase tracking-wider">
                                    {{ str_replace(['Footer — ', 'Footer - '], '', $footerMenu?->name ?? '') }}
                                </h3>
                                @if($footerMenu->location === 'footer_categories' && optional($footerMenu->rootItems)->count() > 6)
                                    <ul class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5">
                                        @foreach($footerMenu?->rootItems ?? [] as $item)
                                            <li>
                                                <a href="{{ url(optional($item)?->url ?? '#') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">
                                                    {{ optional($item)?->title ?? '' }}
                                                </a>
                                            </li>
                                        @endforeach
            ...
            @else
                {{-- Fallback for when no menus are configured --}}
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        <h3 class="text-[13px] font-bold text-text-primary light:text-slate-900 mb-4 uppercase tracking-wider">Company</h3>
                        <ul class="space-y-2.5">
                            <li><a href="{{ route('about') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">About Us</a></li>
                            <li><a href="{{ route('contact') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Contact Us</a></li>
                            <li><a href="{{ route('careers.index') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Careers</a></li>
                        </ul>
                    </div>
                    <div>
                        <h3 class="text-[13px] font-bold text-text-primary light:text-slate-900 mb-4 uppercase tracking-wider">Resources</h3>
                        <ul class="space-y-2.5">
                            <li><a href="{{ route('advertise') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Advertise</a></li>
                            <li><a href="{{ route('press-kit') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Press Kit</a></li>
                        </ul>
                    </div>
                    <div>
                        <h3 class="text-[13px] font-bold text-text-primary light:text-slate-900 mb-4 uppercase tracking-wider">Legal</h3>
                        <ul class="space-y-2.5">
                            <li><a href="{{ route('privacy') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Privacy Policy</a></li>
                            <li><a href="{{ route('terms') }}" class="text-[13px] font-medium text-text-muted light:text-slate-600 hover:text-electric light:hover:text-electric transition-colors">Terms of Service</a></li>
                        </ul>
                    </div>
                </div>
            @endif
        ...
        <div class="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
            <p class="text-text-muted light:text-slate-500 text-xs shrink-0">Copyright © {{ date('Y') }} {{ $siteName ?? 'Atomni' }}. All Rights Reserved.</p>
            <p class="text-text-muted light:text-slate-500 text-xs shrink-0 italic max-w-lg hidden sm:block">
                {{ \App\Models\Setting::get('website_tagline', 'Your trusted source for breaking news and in-depth analysis.') }}
            </p>
        </div>
    </div>
</footer>
