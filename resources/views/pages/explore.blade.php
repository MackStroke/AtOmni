...
@section('title', 'Explore Topics — Atomni')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-16">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" id="categories-grid">
            @forelse($categories as $category)
                <a href="{{ route('category', $category->slug) }}" class="topic-item category-item group bg-navy-900 border border-navy-700 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:bg-navy-800 transition-colors shadow-sm" data-name="{{ strtolower($category->name) }}">
                    <h3 class="font-heading font-semibold text-text-primary group-hover:text-electric-light transition-colors mb-1">{{ $category->name }}</h3>
                    <span class="text-xs font-medium px-2 py-1 bg-navy-900 rounded-md text-text-muted">{{ $category->posts_count }} posts</span>
                </a>
            @empty
    ...
    <div>
        ...
        <div class="flex flex-wrap gap-3" id="tags-cloud">
            @forelse($tags as $tag)
                <a href="{{ route('search', ['tag' => $tag->name]) }}" class="topic-item tag-item px-4 py-2 rounded-full text-sm font-medium text-text-secondary bg-navy-800 border border-navy-700 hover:bg-electric hover:text-white hover:border-electric transition-all flex items-center gap-2" data-name="{{ strtolower($tag->name) }}">
                    #{{ $tag->name }}
