...
?>
<html lang="en" class="scroll-smooth theme-<?php echo e(\App\Models\Setting::get('theme_color', 'blue')); ?>" style="<?php echo e($themeVars); ?>">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo $__env->yieldContent('title', 'Admin'); ?> — Atomni</title>
...
<body class="bg-navy-950 text-text-primary font-sans antialiased min-h-screen transition-colors duration-300 overflow-x-hidden max-w-full">
        ...
        <div class="flex-1 flex flex-col md:ml-64 min-w-0 w-full">
            ...
            <header class="sticky top-0 z-30 w-full max-w-full bg-navy-900/80 backdrop-blur-xl border-b border-navy-700/50 px-4 sm:px-6 py-3 sm:py-4 light:bg-white/80 light:border-slate-200">
                <div class="flex items-center justify-between min-w-0 gap-2">
                    <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                        ...
                        <h1 class="font-heading font-bold text-lg text-text-primary page-title truncate"><?php echo $__env->yieldContent('page-title', 'Dashboard'); ?></h1>
                    </div>
                    <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                        ...
                        <div class="relative pl-3 border-l border-navy-700/50">
                            <button id="user-menu-button" class="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-accent-blue focus:ring-offset-2 focus:ring-offset-navy-950 rounded-lg p-1 transition-colors hover:bg-navy-800/60" aria-expanded="false" aria-haspopup="true">
                                <?php if(auth()->user()->profile_image): ?>
                                    <div class="w-8 h-8 rounded-full overflow-hidden border border-navy-700/50">
                                        <img loading="lazy" src="<?php echo e(\Illuminate\Support\Str::startsWith(auth()->user()->profile_image, 'http') ? auth()->user()->profile_image : \Illuminate\Support\Facades\Storage::url(auth()->user()->profile_image)); ?>" alt="Profile" class="w-full h-full object-cover" onerror="this.parentElement.outerHTML=`<div class='w-8 h-8 rounded-full bg-electric/20 flex items-center justify-center shrink-0'><span class='text-electric text-sm font-semibold'><?php echo e(substr(auth()->user()?->name ?? 'A', 0, 1)); ?></span></div>`">
                                    </div>
                                <?php else: ?>
                                    <div class="w-8 h-8 rounded-full bg-electric/20 flex items-center justify-center shrink-0">
                                        <span class="text-electric text-sm font-semibold"><?php echo e(substr(auth()->user()?->name ?? 'A', 0, 1)); ?></span>
                                    </div>
                                <?php endif; ?>
                                <span class="hidden sm:block text-sm text-text-secondary"><?php echo e(auth()->user()?->name ?? 'Admin'); ?></span>
                            ...
                            <div id="user-menu-dropdown" class="absolute right-0 mt-2 w-48 rounded-xl bg-navy-900 border border-navy-700/50 shadow-xl shadow-black/20 origin-top-right transition-all duration-200 opacity-0 invisible scale-95 pointer-events-none z-50">
                                <div class="p-1">
                                    <a href="<?php echo e(route('admin.profile.edit')); ?>" class="flex items-center gap-2 px-3 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-navy-800 rounded-lg transition-colors">
            ...
            <?php if(session('success')): ?>
                <div class="mx-4 sm:mx-6 mt-4 px-4 py-3 rounded-xl bg-success/10 border border-success/30 text-success text-sm flex items-center gap-2 flash-message">
                    ...
                    <span class="flex-1 min-w-0"><?php echo e(session('success')); ?></span>
                </div>
            <?php endif; ?>
            <?php if(session('error')): ?>
                <div class="mx-4 sm:mx-6 mt-4 px-4 py-3 rounded-xl bg-alert-red/10 border border-alert-red/30 text-alert-red text-sm flex items-center gap-2 flash-message">
                    ...
                    <span class="flex-1 min-w-0"><?php echo e(session('error')); ?></span>
                </div>
            <?php endif; ?>
            <?php if(session('warning')): ?>
                <div class="mx-4 sm:mx-6 mt-4 px-4 py-3 rounded-xl bg-amber/10 border border-amber/30 text-amber text-sm flex items-center gap-2 flash-message">
                    ...
                    <span class="flex-1 min-w-0"><?php echo e(session('warning')); ?></span>
                </div>
            <?php endif; ?>
            <?php if($errors->any()): ?>
    ...
    <?php if (isset($component)) { $__componentOriginal41b0d1c46db7f2dfc8b7a15fae3bce27 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal41b0d1c46db7f2dfc8b7a15fae3bce27 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.media-selector','data' => []] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('admin.media-selector'); ?>
...
<?php endif; ?>
<?php if (isset($__attributesOriginal41b0d1c46db7f2dfc8b7a15fae3bce27)): ?>
<?php $attributes = $__attributesOriginal41b0d1c46db7f2dfc8b7a15fae3bce27; ?>
<?php unset($__attributesOriginal41b0d1c46db7f2dfc8b7a15fae3bce27); ?>
<?php endif; ?>
<?php if (isset($__componentOriginal41b0d1c46db7f2dfc8b7a15fae3bce27)): ?>
<?php $component = $__componentOriginal41b0d1c46db7f2dfc8b7a15fae3bce27; ?>
<?php unset($__componentOriginal41b0d1c46db7f2dfc8b7a15fae3bce27); ?>
<?php endif; ?>
...
<?php /**PATH D:\xampp\htdocs\atomni-pro\resources\views/admin/layouts/app.blade.php ENDPATH**/ ?>